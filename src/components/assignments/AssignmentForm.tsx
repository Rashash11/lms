...
export default function AssignmentForm({ role, initialData, onSuccess }: AssignmentFormProps) {
    ...
    const fetchAllUsers = async () => {
        try {
            ...
            const learners = (data.users || []).filter((u: any) => u.activeRole === 'LEARNER');
    ...
    const fetchInstructors = async () => {
        try {
            ...
            const res = await fetch('/api/users');
            const data = await res.json();
            // Filter to instructors and super instructors
            const instructors = (data.users || []).filter((u: any) =>
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const formatDate = (d: string) => d ? new Date(d).toISOString() : null;
