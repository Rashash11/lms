...
async function runTests() {
    ...
    try {
        ...
        if (res.status === 200 && data.ok && rotated) {
            ...
            logResult('3. Refresh (rotation)', false, `status=${res.status}, rotated=${rotated}`);
        }
    } catch (e: any) {
    ...
    try {
        const res = await fetch(`${BASE_URL}/api/auth/refresh`, {
            method: 'POST',
            headers: {
                'Cookie': `refreshToken=${oldRefreshToken}; session=${cookies['session']}`
            },
        });
        ...
        if (res.status === 401 && !data.ok) {
            logResult('4. Old refresh token rejected', true, 'Correctly returns 401');
        } else {
            logResult('4. Old refresh token rejected', false, `status=${res.status}, should be 401`);
        }
    } catch (e: any) {
        logResult('4. Old refresh token rejected', false, e.message);
    }
    ...
    try {
        ...
        if (res.status === 200 && data.ok) {
            ...
            logResult('5. Logout all', false, `status=${res.status}, ok=${data.ok}`);
        }
    } catch (e: any) {
