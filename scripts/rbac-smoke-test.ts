...
const TEST_ACCOUNTS = {
    ...
    LEARNER: { email: 'learner1@portal.com', password: 'Learner123!' },
};
...
async function runTests() {
    ...
    try {
        ...
        const res = await fetch(`${BASE_URL}/api/learning-paths`, {
            ...
            headers: {
                'Content-Type': 'application/json',
                'Cookie': buildCookieHeader(instructorCookies),
            },
            body: JSON.stringify({
    ...
    try {
        ...
        const res = await fetch(`${BASE_URL}/api/learning-paths`, {
            ...
            headers: {
                'Content-Type': 'application/json',
                'Cookie': buildCookieHeader(learnerCookies),
            },
            body: JSON.stringify({
    ...
    try {
        ...
        const res = await fetch(`${BASE_URL}/api/submissions`, {
            headers: { 'Cookie': buildCookieHeader(instructorCookies) },
        });
    ...
    try {
        ...
        const res = await fetch(`${BASE_URL}/api/reports`, {
            headers: { 'Cookie': buildCookieHeader(adminCookies) },
        });
        if (res.status === 200) {
    ...
    try {
        if (!learnerCookies) throw new Error('No learner session');
        const res = await fetch(`${BASE_URL}/api/reports`, {
            headers: { 'Cookie': buildCookieHeader(learnerCookies) },
        });
        if (res.status === 403) {
