...
const TEST_ACCOUNTS = {
    LEARNER1: { email: 'learner1@portal.com', password: 'Learner123!' },
    LEARNER2: { email: 'learner2@portal.com', password: 'Learner123!' }, // Same password as learner1
...
async function runCertificatesSmoke() {
    ...
    try {
        ...
        const templatesRes = await fetch(`${BASE_URL}/api/admin/certificates/templates`, {
        ...
        if (templates.length > 0) {
            ...
            // Create a template
            const createTemplateRes = await fetch(`${BASE_URL}/api/admin/certificates/templates`, {
                method: 'POST',
                headers: {
        ...
        const enrollmentsRes = await fetch(`${BASE_URL}/api/learner/enrollments`, {
        ...
        const assignRes = await fetch(`${BASE_URL}/api/admin/courses/${courseId}/certificate-template`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                Cookie: adminCookies,
        ...
        const completeRes = await fetch(`${BASE_URL}/api/learner/courses/${courseId}/complete`, {
            method: 'POST',
            headers: { Cookie: learner1Cookies, ...SKIP_AUTH_HEADER }
        });
        ...
        // 5. List certificates
        const certListRes = await fetch(`${BASE_URL}/api/learner/certificates`, {
            headers: { Cookie: learner1Cookies, ...SKIP_AUTH_HEADER }
        });
        ...
        const certDetailRes = await fetch(`${BASE_URL}/api/learner/certificates/${certificateId}`, {
            headers: { Cookie: learner1Cookies, ...SKIP_AUTH_HEADER }
        });
        ...
        const downloadRes = await fetch(`${BASE_URL}/api/learner/certificates/${certificateId}/download`, {
            headers: { Cookie: learner1Cookies, ...SKIP_AUTH_HEADER }
        });
        ...
        if (learner2Cookies) {
            const leakRes = await fetch(`${BASE_URL}/api/learner/certificates/${certificateId}`, {
